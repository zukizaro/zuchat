...
public class ServiceUtils {
    ...
    public static void updateUserStatus(Context context){
        if(isNetworkConnected(context)) {
            ...
            if (!uid.equals("")) {
                FirebaseDatabase.getInstance().getReference().child("users/" + uid + "/status/isOnline").setValue(true);
                FirebaseDatabase.getInstance().getReference().child("users/" + uid + "/status/timestamp").setValue(System.currentTimeMillis());
            }
        }
    }
    ...
    public static void updateFriendStatus(Context context, ListFriend listFriend){
        if(isNetworkConnected(context)) {
            for (Friend friend : listFriend.getListFriend()) {
                ...
                FirebaseDatabase.getInstance().getReference().child("users/" + fid + "/status").addListenerForSingleValueEvent(new ValueEventListener() {
                    @Override
